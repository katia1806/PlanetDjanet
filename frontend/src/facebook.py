...
def show_facebook_dashboard():
    ...
    with col2:
        time_frame = st.selectbox("Select weekly or monthly reaches", ("weekly", "monthly"))

    # Data Aggregation
    agg_func = by_week if time_frame == "weekly" else by_month
    selected_data_streamlit = agg_func(df_likes_fb, year, "Mentions J’aime de la Page Facebook")

    # Displaying Bar Chart
    fig3 = px.bar(selected_data_streamlit, x=time_frame[:-2], y="Mentions J’aime de la Page Facebook")
